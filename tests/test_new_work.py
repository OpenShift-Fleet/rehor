...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "presets" / "shared" / "preflight"))
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / ".claude" / "skills"))

from common import build_repo_lookup
from jira_sprint_preflight import _match_repo_labels
...
class TestMatchRepoLabels:
    ...
    def test_bare_label(self):
        ...
        assert _match_repo_labels(labels, self.lookup) == ["insights-chrome"]

...
## How fixtures are shared between Python and TypeScript

`api_payloads.py` is the **single source of truth** for all fixture data.

```
api_payloads.py  ──►  generate_json.py  ──►  src/test/fixtures.json  ──►  TS tests
      │
      └──►  Python tests (import directly)
      └──►  mock_api.py (imports directly)
```

`fixtures.json` is a **generated file** — do not edit it by hand. Regenerate it
whenever you change default datasets in `api_payloads.py`:

```bash
cd dashboard
python3 tests/fixtures/generate_json.py
```

Commit the updated `fixtures.json` alongside your `api_payloads.py` changes.

## Adding New Fixtures

1. **Add factory function and default dataset** to `fixtures/api_payloads.py`:
...
3. **Add to `generate_json.py`** so TypeScript tests get the new data:

```python
from api_payloads import ..., ISSUES
...
4. **Regenerate `fixtures.json`** and commit it:

```bash
python3 dashboard/tests/fixtures/generate_json.py
git add dashboard/src/test/fixtures.json
```

5. **Use** in Python tests, mock server, and TypeScript helpers:
...
```typescript
// TypeScript — add a maker in src/test/helpers.ts
import fixtures from './fixtures.json';
export function makeIssue(overrides = {}) {
  return { ...fixtures.issues[0], ...overrides };
}
```

...
- **OpenAI-compatible auth proxy** (port 8450, optional) — the same pattern for OpenAI Chat Completions. The proxy overwrites the inbound `Authorization` header with the real key, enforces `OPENAI_ALLOWED_MODELS` from the request body, and only serves `/healthz`, `/v1/models`, and `/v1/chat/completions`. Off unless `OPENAI_API_KEY` is set; the bot never sees the key.
